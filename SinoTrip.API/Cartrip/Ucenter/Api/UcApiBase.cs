...
namespace SinoTrip.API.Cartrip.Ucenter.Api
{
    ...
    public abstract class UcApiBase : IHttpHandler, IRequiresSessionState
    {
        ...
        private void switchAction(HttpContext context)
        {
        ...
        private void synLogin(HttpContext context)
        {
            ...
            int uid;
            int.TryParse(Args.QueryString["uid"], out uid);
            writeEnd(SynLogin(uid, context));
        }
        private void synLogout(HttpContext context)
        {
            if (!UcConfig.ApiSynLogout) writeForbidden();
            Response.Headers.Add("P3P", "CP=\"CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR\"");
            writeEnd(SynLogout(context));
        }
        ...
        /// <param name="uid">uid</param>
        /// <returns></returns>
        public abstract ApiReturn SynLogin(int uid, HttpContext context);
        /// <summary>
        /// 同步登出
        /// </summary>
        /// <returns></returns>
        public abstract ApiReturn SynLogout(HttpContext context);
        /// <summary>
        /// 更新密码
        /// </summary>
